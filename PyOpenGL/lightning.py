...
vert_shader = """
  ...
  void main()
  {
      ...
      vec3 vertex_pos = vec3(transform * vec4(position, 1.0f));
      vec3 lightDirection = normalize(light_direction - vertex_pos);
...
def on_draw():
    ...
    glBindVertexArray(qvao)
    qt1 = pyrr.matrix44.create_from_translation([lightPos[0], lightPos[1], lightPos[2]])
    qt2 = pyrr.matrix44.create_from_scale([.2, .2, .2])
...
def on_key_press(key, modifiers):
    global w_pressed, a_pressed, s_pressed, d_pressed, t_pressed, g_pressed, rotating
...
def on_key_release(key, modifiers):
    global w_pressed, a_pressed, s_pressed, d_pressed, t_pressed, g_pressed
    if key == pyglet.window.key.W:
        w_pressed = False
    if key == pyglet.window.key.A:
        a_pressed = False
...
def update(dt):
    ...
    if w_pressed:
        lightPos[2] = lightPos[2] + 0.5 * dt
    if s_pressed:
        lightPos[2] = lightPos[2] - 0.5 * dt
    if a_pressed:
        lightPos[0] = lightPos[0] + 0.5 * dt
    if d_pressed:
        lightPos[0] = lightPos[0] - 0.5 * dt
    if t_pressed:
        lightPos[1] = lightPos[1] + 0.5 * dt
    if g_pressed:
        lightPos[1] = lightPos[1] - 0.5 * dt

    glUniform3f(light_direction, lightPos[0], lightPos[1], lightPos[2])
